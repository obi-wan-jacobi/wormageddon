import { Void, isDeepEqual } from '@plasmastrapi/base';
  ...
  public once({ components }: { components: IComponentMaster }): void {
    components.forEvery(WormStateComponent)((wsComponent) => {
      ...
      if (heading === $?.previous?.heading && facing === $.previous.facing && action === $.previous.action) {
        ...
        if ([WORM_ACTION.ARC, WORM_ACTION.AIR].includes(action) && Epsilon.isRoughlyEqual(pose as any, pose.$!.previous, 0.005)) {
...
const ACTIONS_MAP: { [key: number]: Void<{ worm: Worm; f: 1 | -1; facing: string; heading: string }> } = {
  [WORM_ACTION.IDLE]: ({ worm, facing, heading }: { worm: Worm; facing: string; heading: string }) => {
    ...
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/idle/wbrth1${facing}${heading}.png`, 60, 60, 1200, { x: -30, y: -30 }),
      frame: 0,
      isRollback: true,
      durationMs: 30,
    });
  ...
  [WORM_ACTION.WALK]: ({ worm, f, facing, heading }: { worm: Worm; f: 1 | -1; facing: string; heading: string }) => {
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/walk/wwalk${facing}${heading}.png`, 60, 60, 900, { x: -30, y: -30 }, f * 0.667),
      frame: 0,
      durationMs: 30,
    });
  },
  [WORM_ACTION.JUMP]: ({ worm, facing, heading }: { worm: Worm; facing: string; heading: string }) => {
    ...
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/jump/wjump${facing}${heading}.png`, 60, 60, 600, { x: -30, y: -30 }),
      frame: 0,
      durationMs: 20,
    });
  },
  [WORM_ACTION.ARC]: ({ worm, facing }: { worm: Worm; facing: string }) => {
    ...
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/arc/warc${facing}.png`, 60, 60, 420, { x: -30, y: -30 }),
      frame: 0,
      durationMs,
    });
  },
  [WORM_ACTION.AIR]: ({ worm, facing }: { worm: Worm; facing: string }) => {
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/fly/wfly${facing}D.png`, 60, 60, 60, { x: -30, y: -30 }),
      frame: 0,
      durationMs: 0,
    });
  },
  [WORM_ACTION.LAND]: ({ worm, facing, heading }: { worm: Worm; facing: string; heading: string }) => {
    ...
    worm.$add(AnimationComponent, {
      images: buildFrames(`./assets/land/wland1${facing}${heading}.png`, 60, 60, 360, { x: -30, y: -30 }),
      frame: 0,
      durationMs,
    });
  },
